...
Commit and branch names are currently built from fixed templates with detail pulled straight from what actually changed — an automatic commit is named by its own summary, "3 clips moved, 1 trimmed", counted the same way the Changes view counts. AI naming was deliberately left out of version 1 for the same reason AI was kept out of the diff engine: a template can never be wrong, but an AI-generated name can — and a misleading name in a version-control history is a trust problem, not a cosmetic one.
