...
**Deciding conflicts.** Nothing is stored on the server while a person decides. Their answers are held in the browser and sent up with every request, and the entire merge result is recalculated from the beginning from those answers — never by editing a half-finished draft. Clicking conflicts in a different order never changes the outcome, and re-answering one is allowed. Only the final action writes anything: it re-checks that neither side moved while the person was deciding, then recomputes and commits the result in a single transaction.
