...
**Two people at once.** Each browser tab posts to `/api/sync` every three seconds: it says which cut it is on and where its playhead sits, and gets back everything that happened on the project since it last asked, plus the other tabs alive in the last ten seconds. That is the whole collaboration channel — no push connection, and no co-editing of a single cut: two people work on two cuts, and each sees the other's work arrive within about three seconds. Presence is display-only: a peer's coloured playhead and the `is on ‹cut›` chip are drawn from it, and no server decision ever consults it, so a dropped heartbeat can never turn into a lost edit. "Ready for main" works the same way — it is a **mark on a cut**, with a one-line note from whoever made it, not a lock: main stays editable, anyone can still bring the cut in or not, and the mark clears itself once the cut lands.
