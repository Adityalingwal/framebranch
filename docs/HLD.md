...
**Handling collisions.** Nothing is locked while editing. Each save checks: is the branch still where I started? If yes, it saves normally. If not, the save is rejected — nothing is lost or silently overwritten. The interface rolls back its preview, refetches the latest timeline, and says so: "Timeline updated — someone else edited this cut." The edit has to be redone on the fresh state.
...
- **Request failures are not all handled the same way:**
  ...
  | Retries exhausted | Banner shows: "Connection lost — your work is safe". Editing pauses; a manual Retry is offered. No offline mode. |
...
**Two people at once.** Each browser tab posts to `/api/sync` every three seconds: it says which cut it is on and where its playhead sits, and gets back everything that happened on the project since it last asked, plus the other tabs alive in the last ten seconds. That "everything" is an append-only feed of project events — a version created, a cut created or switched, a restore, a bring-in landed, an import, a ready mark set or cleared, and a plain `edit` (nine kinds in all) — read forward from a cursor, so a tab that was away for a while catches up from where it stopped — 200 events per answer, the rest on the next tick — and never replays what it already saw. That is the whole collaboration channel — no push connection, and no co-editing of a single cut: two people work on two cuts, and each sees the other's work arrive within about three seconds. Presence is display-only: a peer's coloured playhead and the `is on ‹cut›` chip are drawn from it, and no server decision ever consults it, so a dropped heartbeat can never turn into a lost edit. "Ready for main" works the same way — it is a **mark on a cut**, with a one-line note from whoever made it, not a lock: main stays editable, anyone can still bring the cut in or not, and the mark clears itself once the cut lands.

**What would need to be added for a real product.** This version runs for two participants at a time on one project — a person and an agent, or two people in two browser tabs — and identity is a name typed into a box, so there is no rate limiting, no accounts and no permissions; the interface itself is only built and polished for a small, demo-sized timeline (10-30 clips), even though the engine underneath is proven at 10,000. Turning this into a real multi-user product would mean adding: a way to limit how fast requests can come in, login and permissions, a push channel in place of the three-second poll, and an interface that stays fast with far more clips on screen. The underlying architecture doesn't need to change for any of this — the API and engine are already stateless, so they scale by running more copies, not by being rewritten.
